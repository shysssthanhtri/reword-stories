...
The root `AppRouter` in `src/server/trpc/router.ts` SHALL register a `translations` feature router exposing `listProviders`, `estimateChunks`, `create`, `getById`, `listByChapter`, `retry`, `retryChunk`, and `delete` procedures.
