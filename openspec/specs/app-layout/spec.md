...
The application SHALL serve placeholder Server Component pages at `/novels` and `/settings` with page titles matching the route. The root path `/` SHALL redirect to `/novels`.
...
#### Scenario: Placeholder pages render inside shell

- **WHEN** user visits `/novels` or `/settings`
