...
Translation job steps SHALL log with prefix `[translation-job]` (or the shared queue log prefix if renamed). Operators SHALL be able to inspect job runs in the Vercel dashboard under Observability → Workflows.
